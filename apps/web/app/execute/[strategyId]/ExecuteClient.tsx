...
interface ExecuteClientProps {
  strategyId: string;
  /** Pre-filled by an agent intent (?amount=). */
  initialAmount?: string;
  initialPath?: Path;
  /** True when the user arrived from an agent intent (?via=agent). */
  viaAgent?: boolean;
}
...
export function ExecuteClient({
  ...
  const [path, setPath] = useState<Path>(initialPath ?? "fsa");
  ...
  return (
    <div className="container-app py-10">
      <div className="mx-auto max-w-3xl">
        ...
        {viaAgent && initialAmount && (
          <div className="mt-4 rounded-xl border border-brand/40 bg-brand/10 p-3 text-[13px] leading-snug text-ink-soft">
            <span className="font-mono text-[10px] font-semibold uppercase tracking-wider text-brand">
              intent from the copilot
            </span>{" "}
            — the amount ({initialAmount} FXRP) and path were pre-filled from your
            conversation. Review everything before signing; you are always in control.
          </div>
        )}
        ...
        <div className="mt-4" role="tabpanel">
          {path === "fsa" ? (
            <FsaDepositFlow strategy={strategy} initialAmount={initialAmount} />
          ) : (
            <EvmDepositFlow strategy={strategy} initialAmount={initialAmount} />
          )}
        </div>
